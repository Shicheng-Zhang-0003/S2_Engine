...
int sim_place_dipeptide_GlyAla(Simulation *sim, Vec3 origin, int *out_ala_N) {
    ...
    /*
     * Add the angle terms SPANNING the new peptide bond junction -
     * these are otherwise entirely missing (angles are only built
     * ONCE per individual amino acid, before any inter-residue bonds
     * exist). Without these, nothing constrains the local geometry
     * across a peptide bond junction except bond length and whatever
     * dihedral terms happen to be present - discovered as the root
     * cause of a real, extended failure for poly-alanine chains to
     * converge toward target helical dihedral angles.
     *
     * Values are well-established average protein backbone geometry
     * (Engh & Huber-type parameters, standard throughout structural
     * biology) - not independently verified via primary-source fetch
     * in this session, same confidence category as the 1.33 A peptide
     * bond length already used above.
     */
    {
        const double DEG2RAD_LOCAL = 3.14159265358979323846 / 180.0;
        const double k_junction_angle = 3.5;
        int gly_CA = gly + 1, gly_O = gly + 3;
        int ala_CA = ala_N + 1, ala_H = ala_N + 6;

        sim_add_angle_explicit(sim, gly_CA, gly_C, ala_N,
                                116.6 * DEG2RAD_LOCAL, k_junction_angle);
        sim_add_angle_explicit(sim, gly_O, gly_C, ala_N,
                                122.7 * DEG2RAD_LOCAL, k_junction_angle);
        sim_add_angle_explicit(sim, gly_C, ala_N, ala_CA,
                                121.7 * DEG2RAD_LOCAL, k_junction_angle);
        sim_add_angle_explicit(sim, gly_C, ala_N, ala_H,
                                119.0 * DEG2RAD_LOCAL, k_junction_angle);
    }
...
/* ══════════════════════════════════════════════════════════════════════════
 * Centralized, defensive index tracking for chain assembly
 * ══════════════════════════════════════════════════════════════════════════ */
void aa_adjust_residues_after_removal(AAResidue *residues, int count,
    ...
    for (int j = 0; j < count; j++) {
        ...
        if (residues[j].C   > removed_idx) residues[j].C--;
        if (residues[j].O   > removed_idx) residues[j].O--;
        if (residues[j].OXT > removed_idx) residues[j].OXT--;
        if (residues[j].HXT > removed_idx) residues[j].HXT--;
        if (residues[j].H2  > removed_idx) residues[j].H2--;
        if (residues[j].H   > removed_idx) residues[j].H--;
    }
}
 ...
 * POLY-ALANINE CHAIN
 *
 * All n_residues alanine molecules are placed FIRST (each at a well-
 * separated temporary position), before any removal happens. Bonds
 * are then formed sequentially, with a FULL defensive index-
 * adjustment pass across ALL residues after EVERY removal (not just
 * the 2 residues directly involved), and the angle terms spanning
 * each new peptide bond junction are added immediately (the missing-
 * angle bug found and fixed above, applied here from the start).
 * ══════════════════════════════════════════════════════════════════════════ */
int sim_place_polyalanine(Simulation *sim, Vec3 origin, int n_residues,
    ...
    for (int i = 0; i < n_residues; i++) {
        int first = sim_place_alanine(sim, vec3_add(origin, vec3(i * 12.0, 0.0, 0.0)));
        res[i].N   = first + 0;
        res[i].CA  = first + 1;
        res[i].C   = first + 2;
        res[i].O   = first + 3;
    ...
    for (int i = 0; i < n_residues - 1; i++) {
        Vec3 C_pos = sim->atoms[res[i].C].position;
        Vec3 to_CA = vec3_normalize(vec3_sub(sim->atoms[res[i].CA].position, C_pos));
        Vec3 to_O  = vec3_normalize(vec3_sub(sim->atoms[res[i].O].position, C_pos));
        Vec3 open_dir = vec3_normalize(vec3_negate(vec3_add(to_CA, to_O)));

        Vec3 N_pos  = sim->atoms[res[i+1].N].position;
        Vec3 H2_pos = sim->atoms[res[i+1].H2].position;
        Vec3 NH2_dir = vec3_normalize(vec3_sub(H2_pos, N_pos));
        Vec3 target_dir = vec3_negate(open_dir);
        ...
        int block_first = res[i+1].N;
        int block_natoms = sim->num_atoms - block_first;
        nb_transform_rigid(sim, block_first, block_natoms, N_pos, axis, angle, vec3_zero());

        Vec3 target_N = vec3_add(C_pos, vec3_scale(open_dir, PEPTIDE_BOND_LEN));
        Vec3 N_now = sim->atoms[res[i+1].N].position;
        Vec3 translation = vec3_sub(target_N, N_now);
        nb_transform_rigid(sim, block_first, block_natoms, N_pos, vec3_zero(), 0.0, translation);

        int targets[3] = {res[i].OXT, res[i].HXT, res[i+1].H2};
        for (int x = 0; x < 3; x++)
            for (int y = x+1; y < 3; y++)
                if (targets[y] > targets[x]) {
                    int t = targets[x]; targets[x] = targets[y]; targets[y] = t;
                }
        ...
        sim_add_bond(sim, res[i].C, res[i+1].N, 1);

        /* Junction angles (see the dipeptide's comment above for the
         * full discovery/reasoning), added from the start this time. */
        const double DEG2RAD_LOCAL = 3.14159265358979323846 / 180.0;
        const double k_junction_angle = 3.5;
        sim_add_angle_explicit(sim, res[i].CA, res[i].C, res[i+1].N,
                                116.6 * DEG2RAD_LOCAL, k_junction_angle);
        sim_add_angle_explicit(sim, res[i].O, res[i].C, res[i+1].N,
                                122.7 * DEG2RAD_LOCAL, k_junction_angle);
        sim_add_angle_explicit(sim, res[i].C, res[i+1].N, res[i+1].CA,
                                121.7 * DEG2RAD_LOCAL, k_junction_angle);
        sim_add_angle_explicit(sim, res[i].C, res[i+1].N, res[i+1].H,
                                119.0 * DEG2RAD_LOCAL, k_junction_angle);
    }

    for (int i = 0; i < n_residues; i++) residues_out[i] = res[i];
    return res[0].N;
}
