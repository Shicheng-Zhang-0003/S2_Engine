...
Simulation *sim_create(int atom_capacity, int bond_capacity) {
    ...
    /* Dihedrals: heuristic - up to 3 x bonds, same as angles. This
     * array was previously declared in the struct and freed in
     * sim_destroy but NEVER ALLOCATED here - a real, latent
     * NULL-pointer bug, harmless only because nothing had ever tried
     * to populate sim->dihedrals until dihedral force support was
     * added. Fixed now that it's actually used. */
