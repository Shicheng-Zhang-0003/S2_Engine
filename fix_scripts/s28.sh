#!/usr/bin/env bash
set -euo pipefail
# s28: fix G-C coplanarity. The ring-plane alignment makes the rings
# parallel but not coplanar - cytosine ends up 0.75 A offset in the
# ring-normal (Y) direction from guanine, causing sub-1-A close
# contacts that the minimizer resolves by breaking the H-bonds.
# Fix: after ring-plane alignment, translate cytosine in Y to make
# it coplanar with guanine, BEFORE the azimuthal rotation and
# H-bond distance translation.
echo "=== s28: fix G-C coplanarity (Y-offset after ring alignment) ==="
...
# Find the demo_dna_duplex function
...
/* Align ring normals (make rings parallel) */
...
/* COPLANARITY FIX: the ring-plane alignment above makes the rings
* parallel but NOT coplanar. The cytosine ring ends up offset in
* the ring-normal (Y) direction from guanine. For a proper
* Watson-Crick pair, both bases must be in the same plane.
* Translate cytosine in Y to make it coplanar with guanine. */
{
    Vec3 g_centroid = vec3_zero();
    for (int i = 0; i < 3; i++)
        g_centroid = vec3_add(g_centroid, sim->atoms[g_ring[i]].position);
    g_centroid = vec3_scale(g_centroid, 1.0/3.0);

    Vec3 c_centroid = vec3_zero();
    for (int i = 0; i < 3; i++)
        c_centroid = vec3_add(c_centroid, sim->atoms[c_ring[i]].position);
    c_centroid = vec3_scale(c_centroid, 1.0/3.0);

    double y_offset = g_centroid.y - c_centroid.y;
    Vec3 coplanar_trans = vec3(0.0, y_offset, 0.0);
    nb_transform_rigid(sim, c, 13, c_pivot, vec3_zero(), 0.0, coplanar_trans);
}
...
/* COPLANARITY FIX for A-T pair */
{
    Vec3 a_centroid = vec3_zero();
    for (int i = 0; i < 3; i++)
        a_centroid = vec3_add(a_centroid, sim->atoms[a_ring[i]].position);
    a_centroid = vec3_scale(a_centroid, 1.0/3.0);

    Vec3 t_centroid = vec3_zero();
    for (int i = 0; i < 3; i++)
        t_centroid = vec3_add(t_centroid, sim->atoms[t_ring[i]].position);
    t_centroid = vec3_scale(t_centroid, 1.0/3.0);

    double y_offset_at = a_centroid.y - t_centroid.y;
    Vec3 coplanar_trans_at = vec3(0.0, y_offset_at, 0.0);
    nb_transform_rigid(sim, t, 15, t_pivot, vec3_zero(), 0.0, coplanar_trans_at);
}

/* Azimuthal rotation for A-T */
...
printf("  Placed: G-C at y=0, A-T at y=%.1f (stacked along ring normal)\n", rise);
...
with open('src/main.c', 'w') as f:
    f.write(src)
print('  [OK] demo_dna_duplex replaced with coplanarity-fixed version.')
PYEOF
