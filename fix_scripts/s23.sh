#!/usr/bin/env bash
set -euo pipefail
echo "=== s23: fix helical twist pivot to A-T pair center ==="
...
/* ── Helical twist: rotate A-T pair around the PAIR CENTER ────────── */
/* s22 bug: pivot was at adenine's position (0,0,rise), which is NOT
* the A-T pair center. Rotating around the wrong point shears the
* internal geometry, creating a 0.165 A overlap (A:N7 vs T:HN3).
* Fix: pivot at the midpoint of A:N1 and T:N3, which is the actual
* geometric center of the base pair. A rigid rotation around this
* point preserves all internal A-T distances. */
...
print('  [OK] demo_dna_duplex replaced with corrected twist pivot.')
PYEOF
