#!/usr/bin/env bash
set -euo pipefail
# s31: fix translation direction.
# s20-s30 all translated C:N3 ALONG the WC edge (G:O6->G:N1 direction),
# which puts C:N4 on top of G:N1 after the azimuthal rotation.
# The correct direction is PERPENDICULAR to the WC edge (the H-bond
# direction), computed as ring_normal x wc_edge. This is exactly what
# Demo 7 does with G:N1->G:HN1.
echo "=== s31: fix translation to perpendicular-to-WC-edge ==="
...
/* ══════════════════════════════════════════════════════════════════
* G-C PAIR  (same strategy as validated Demo 7)
* ══════════════════════════════════════════════════════════════════ */
...
/* Translate C:N3 to 2.95 A from G:N1 along the H-BOND DIRECTION
* (= ring_normal x WC_edge, perpendicular to the WC edge).
* This is exactly what Demo 7 does with G:N1->G:HN1. */
...
/* Ensure hb_gc points away from guanine's ring center */
{
...
/* Translate T:N3 to 2.90 A from A:N1 along the H-BOND DIRECTION */
...
with open('src/main.c', 'w') as f:
    f.write(src)
print('  [OK] demo_dna_duplex replaced with perpendicular-translation version.')
PYEOF
